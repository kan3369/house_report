        ...
        <form action="{{ $action }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="">
                ...
                <input type="text" name="property_number" class="w-full"
                    value="{{ $report?->latestHistory->property_number }}">
            </div>

            <div class="">
                <h2 class="">{{ __('住所') }}</h2>
                <input type="text" name="address" class="w-full" value="{{ $report?->latestHistory->address }}">
            </div>

            <div class="">
                <h2 class="">{{ __('物件名') }}</h2>
                <input type="text" name="property_name" class="w-full"
                    value="{{ $report?->latestHistory->property_name }}">
            </div>
            ...
            <div class="">
                ...
                <input type="date" name="reported_at" class="w-full"
                    value="{{ old('reported_at', $report?->latestHistory->reported_at?->format('Y-m-d')) }}">
            </div>
